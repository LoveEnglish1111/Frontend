...
export default function StudySetCard({ set, onStudyClick, onQuizClick }) {
  ...
  const bgGradient = categoryColors[set.category] || 'from-slate-50 to-slate-100';
  ...
  return (
    <div className="bg-white rounded-2xl border-2 border-border shadow-sm hover:shadow-md transition-all overflow-hidden">
      ...
      <div className={`bg-gradient-to-br ${bgGradient} p-6 pb-4`}>
        ...
        <h3 className="text-xl font-bold text-foreground mb-2">{set.title}</h3>
      ...
      <div className="p-6 space-y-4">
        ...
        <div>
          ...
          <ProgressBar
            current={set.learned}
            total={set.total}
            color={set.learned === set.total ? 'success' : 'primary'}
          />
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 text-center py-3 border-y border-border">
          <div>
            <p className="text-lg font-bold text-foreground">{set.total}</p>
            <p className="text-xs text-muted-foreground">Cards</p>
          </div>
          <div>
            <p className="text-lg font-bold text-primary-600">{set.learned}</p>
            <p className="text-xs text-muted-foreground">Learned</p>
          </div>
          <div>
            <p className="text-lg font-bold text-warning">{set.reviews}</p>
