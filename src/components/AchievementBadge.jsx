...
export default function AchievementBadge({ badge, unlocked = false, progress = 100 }) {
  ...
  return (
    <div className={`relative p-4 rounded-2xl text-center transition-all ${unlocked ? `bg-gradient-to-br ${bgGradient}` : 'bg-slate-100'}`}>
      ...
      <div className="flex justify-center mb-2">
        <div className={`p-3 rounded-full ${unlocked ? 'bg-white' : 'bg-slate-300'}`}>
          <Icon size={24} className={unlocked ? 'text-primary-600' : 'text-slate-600'} />
        </div>
      </div>

      {/* Badge Name */}
      <h4 className={`text-sm font-bold mb-1 ${unlocked ? 'text-foreground' : 'text-slate-600'}`}>
        {badge.name}
      </h4>

      {/* Description */}
      <p className={`text-xs mb-3 ${unlocked ? 'text-muted-foreground' : 'text-slate-500'}`}>
        {badge.description}
      </p>
      ...
      {!unlocked && progress < 100 && (
        <div className="space-y-1">
          ...
          <p className="text-xs font-semibold text-slate-600">{Math.round(progress)}%</p>
        </div>
      )}
