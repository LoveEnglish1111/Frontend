        ...
        return (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                <div className="bg-white rounded-2xl p-8 max-w-md w-full">
                    ...
                    {/* Feedback */}
                    <div
                        className={`p-4 rounded-lg mb-6 ${percentage >= 80 ? 'bg-success bg-opacity-10' : 'bg-warning bg-opacity-10'}`}
                    >
                        <p
                            className={`text-sm font-semibold ${percentage >= 80 ? 'text-success' : 'text-warning'}`}
                        >
                            {percentage >= 90
    ...
    return (
        <div className="max-w-2xl mx-auto p-6">
            ...
            <div className="bg-white rounded-2xl border-2 border-border p-8 mb-8">
                    ...
                    {options.map((option, index) => {
                        ...
                        return (
                            <button
                                key={index}
                                onClick={() =>
                                    !answered && handleAnswer(options, index)
                                }
                                disabled={answered}
                ...
                {answered && (
                    <div
                        className={`mt-6 p-4 rounded-lg ${selectedAnswer === options.indexOf(currentCard.meaning) ? 'bg-success bg-opacity-10' : 'bg-danger bg-opacity-10'}`}
                    >
                        <p
                            className={`text-sm font-semibold ${
                                selectedAnswer ===
                                options.indexOf(currentCard.meaning)
                                    ? 'text-success'
                                    : 'text-danger'
                            }`}
                        >
