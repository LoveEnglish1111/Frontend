...
export default function AdminPanel() {
    ...
    const deleteUser = (userId) => {
        const confirm = window.confirm(
            'Xóa tài khoản này? Hành động này không thể hoàn tác.',
        );
        if (!confirm) return;
    ...
    const warnUser = (userId) => {
        setUsers((prev) =>
            prev.map((u) => {
                ...
                const willSuspend =
                    newWarnings >= 3 && u.status !== 'suspended';
                const updated = {
                    ...u,
                    warnings: newWarnings,
                    status: willSuspend ? 'suspended' : u.status,
                };
                                ...
                                <tbody>
                                    {filteredUsers.map((u) => {
                                        ...
                                        return (
                                            <tr
                                                key={u.id}
                                                className={`${rowBg} border-t border-border`}
                                            >
                            ...
                            {posts.filter((p) => p.status === 'pending')
                                ...
                                <div className="col-span-full bg-white rounded-2xl border-2 border-border p-8 text-center">
                                    ...
                                    <h3 className="text-xl font-bold text-foreground mb-2">
                                        Không có nội dung nào cần kiểm duyệt
                                    </h3>
                                    <p className="text-sm text-muted-foreground">
                                ...
                                posts
                                    ...
                                    .map((p) => {
                                        ...
                                        return (
                                            <div
                                                key={p.id}
                                                className="bg-white rounded-2xl border-2 border-border p-6"
                                            >
